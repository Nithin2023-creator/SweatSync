...
def parse_timeline_to_weeks(timeline: str) -> int:
    """Parse a target_timeline string into total weeks. Minimum 1 week."""
    t = timeline.lower().strip()
    # Extract number
    match = regex.search(r'(\d+)', t)
...
def architect_node(state: SweatSyncState) -> dict:
    ...
    task = Task(
        description=(
            ...
            f"Task: Create a {total_weeks}-week Strategic Blueprint.\n"
            ...
            f"3. Define {total_weeks}-week periodization. Use ~60% Build (RPE 7-8), ~30% Push (RPE 8-9), and the last week as Recover (RPE 5-6). For 1-week plans, use a single Build week.\n"
    ...
    try:
        ...
        blueprint = StrategicBlueprint(
            ...
            periodization=[PeriodizationWeek(week_number=i, phase="Build", volume_modifier=1.0, rpe_ceiling=8) for i in range(1, total_weeks + 1)],
